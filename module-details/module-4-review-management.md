...
- **Mocked GMB Integration**: `fetchGoogleReviews` in `src/services/reviews.ts` is currently returning static mock data (`John Doe`, `Jane Smith`). The true Google Business Profile OAuth scope and API payload parsing are not yet implemented.
- **Post Reply Simulation**: The `/api/reviews/[id]/post-reply` endpoint sets the DB status to `POSTED` but does not actually make an HTTP request to the Google servers to publish the comment.

## 12. Future Scaling Suggestions
- **Review Alerts**: Connect the `ReviewMonitorLog` to an email or Slack integration. If a review is tagged as `critical`, immediately notify the business owner via SMS (Twilio) so they can intervene before the AI draft is even read.
