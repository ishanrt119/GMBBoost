...
export async function processAuditJob(auditId: string) {
  ...
  try {
    ...
    // ── Step 3b: Fetch real SERP rank positions for each competitor ──────────
    // This produces avgRank values like 3.6, 7.4, 9.3 — matching the reference report.
    // Uses the same keywords we already fetched, limited to top 3 for API efficiency.
    let competitorsWithRanks = realCompetitors;
    if (realCompetitors.length > 0 && realKeywordRankings.length > 0) {
      competitorsWithRanks = await fetchCompetitorSerpRanks(
        realCompetitors,
        serpKeywords,
        cityLocation
      );
    ...
    // The overall "Search Rank" shown on the report is the avg SERP rank (like 18.9),
    // NOT a 0-100 quality score. Use overallAvgRank if we have real ranking data.
    // Fall back to quality score only if SERPAPI data is unavailable.
    const reportRank = realMetrics.overallAvgRank > 0
      ? realMetrics.overallAvgRank
      : Math.round(
          (auditDataRest.completenessScore + auditDataRest.engagementScore + auditDataRest.sentimentScore + auditDataRest.keywordScore) / 4
        );

    audit.overallScore   = reportRank;
    audit.competitors    = competitorsWithRanks;
