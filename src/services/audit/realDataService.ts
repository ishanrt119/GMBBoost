...
// ─── SERPAPI: Competitor Rankings ─────────────────────────────────────────────

/**
 * For each competitor, search the same top keywords and find their average
 * rank in the local pack. This produces real SERP rank numbers like the
 * reference report shows (3.6, 7.4, 9.3, etc.).
 */
export async function fetchCompetitorSerpRanks(
  competitors: ICompetitor[],
  keywords: string[],
  location: string
): Promise<ICompetitor[]> {
  if (!SERP_API_KEY || keywords.length === 0 || competitors.length === 0) {
    return competitors;
  }

  const enriched: ICompetitor[] = [];

  for (const competitor of competitors.slice(0, 5)) {
    const lowerComp = competitor.name.toLowerCase().slice(0, 8);
    const ranks: number[] = [];

    for (const keyword of keywords.slice(0, 3)) { // use top 3 keywords for speed
      try {
        const url = new URL('https://serpapi.com/search.json');
        url.searchParams.set('engine', 'google');
        url.searchParams.set('q', keyword);
        ...
        const localPack = data.local_results?.places ?? [];
        const packIdx = localPack.findIndex((p: any) =>
          p.title?.toLowerCase().includes(lowerComp) ||
          competitor.name.toLowerCase().includes((p.title ?? '').toLowerCase().slice(0, 6))
        );

        if (packIdx !== -1) {
          ranks.push(packIdx + 1);
        } else {
          ranks.push(21); // not found in top 20
        }

        await new Promise(r => setTimeout(r, 300));
      } catch {
        // skip this keyword for this competitor
      }
    }

    const avgRank = ranks.length > 0
      ? parseFloat((ranks.reduce((a, b) => a + b, 0) / ranks.length).toFixed(1))
      : 0;

    enriched.push({ ...competitor, avgRank });
  }

  // Sort by avgRank ascending (best ranking first), competitors with 0 rank go last
  return enriched.sort((a, b) => {
    if (a.avgRank === 0) return 1;
    if (b.avgRank === 0) return -1;
...
export function deriveRealMetrics(
  ...
  // ── Overall Average SERP Rank ─────────────────────────────────────────────
  // Average rank across all keywords where rank > 0 (0 = no data/estimated)
  // Rank 21 means "not in top 20" — we treat it as 21 for averaging
  const rankedKeywords = keywordRankings.filter(k => k.rank > 0 && k.source === 'serpapi');
  const overallAvgRank = rankedKeywords.length > 0
    ? parseFloat((rankedKeywords.reduce((sum, k) => sum + k.rank, 0) / rankedKeywords.length).toFixed(1))
    : 0;

  // ── Profile SEO Score (0-100) ─────────────────────────────────────────────
  // Based on whether target keywords appear in key GBP sections.
  // We derive this deterministically from what data we have:
  // - Title/name contains relevant keywords: hard to check without GBP API, use avg rank as proxy
  // - Categories: presence of additional categories
  // - Services: aiServicesCount
  // - Description: hasDescription
  let seoScore = 0;
  // Keyword rank proxy: rank 1-5 = 80-100, rank 6-10 = 60-79, rank 11-20 = 30-59, rank 20+ = 0-29
  if (overallAvgRank > 0) {
    if (overallAvgRank <= 5) seoScore = 85;
    else if (overallAvgRank <= 10) seoScore = 65;
    else if (overallAvgRank <= 15) seoScore = 45;
    else if (overallAvgRank <= 20) seoScore = 30;
    else seoScore = 15;
  } else if (biz.hasDescription) {
    seoScore = 30; // some base score if no ranking data
  }
  // Bonus for additional categories (+10) and services (+10)
  if (aiCategoriesCount > 1) seoScore = Math.min(100, seoScore + 10);
  if (aiServicesCount >= 10) seoScore = Math.min(100, seoScore + 10);

  // ── Missing keyword locations ──────────────────────────────────────────────
  // Determines which GBP sections are missing optimized keywords,
  // shown as red bullet points in the Profile SEO Score card.
  const missingKeywordLocations: string[] = [];
  // Title: if avg rank is poor, keywords likely missing from business name/title
  if (seoScore < 80) missingKeywordLocations.push('Title');
  // Additional Category: if no additional categories
  if (aiCategoriesCount <= 1 || !biz.categories || biz.categories.length <= 1) {
    missingKeywordLocations.push('Additional Category');
  }
  // Services: if services count is low
  if (aiServicesCount < 5) missingKeywordLocations.push('Services');
  // Description: if no description
  if (!biz.hasDescription) missingKeywordLocations.push('Description');
