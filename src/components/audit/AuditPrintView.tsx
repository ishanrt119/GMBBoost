...
export default function AuditPrintView({ auditId }: { auditId: string }) {
  ...
  const reviewsPerWeek  = (sentiment / 100 * 3).toFixed(2);
  const responsePercent = Math.min(99, Math.round(engagement * 0.8));
  const servicesAdded   = Math.max(0, Math.round(profileScore / 8));
  const categoriesAdded = Math.max(0, Math.round(profileScore / 25));
  ...
  // Keyword rows from weaknesses (closest available field)
  const kwRows = (auditData?.weaknesses ?? []).slice(0, 5).map((kw, i) => ({
    keyword: kw,
    rank: Math.round(12 + i * 3 + (i % 2) * 2),
  }));

  // Completion checklist
  const checklist = [
    { label: 'Title',                             done: profileScore >= 10 },
    { label: 'Primary Category',                 done: profileScore >= 20 },
    { label: 'Additional Categories',            done: profileScore >= 45 },
    { label: 'Business Services',                done: profileScore >= 50 },
    { label: 'Description',                      done: seoScore     >= 60 },
    { label: 'Address',                          done: profileScore >= 30 },
    { label: 'Phone',                            done: profileScore >= 30 },
    { label: 'Listing Attributes/Service Options', done: engagement  >= 50 },
    { label: 'Photos',                           done: profileScore >= 60 },
    { label: 'Logo',                             done: profileScore >= 70 },
    { label: 'Website',                          done: profileScore >= 40 },
    { label: 'Service Area',                     done: profileScore >= 50 },
    { label: 'Business Hours',                   done: profileScore >= 20 },
    { label: 'Appointment/Ordering Links',       done: engagement   >= 70 },
  ];
        ...
        <div className="page">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
            ...
            <Card>
              ...
              <div style={{ fontSize: 52, fontWeight: 900, color: '#1e293b', lineHeight: 1 }}>{overallScore}</div>
                    ...
                    <tbody>
                      {kwRows.map((kw, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                          ...
                          <td style={{ padding: '8px 0', fontSize: 15, fontWeight: 900, color: '#1e293b', textAlign: 'right' }}>{kw.rank}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {competitors.slice(0, 5).map((c, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                          ...
                          <td style={{ padding: '8px 0', fontSize: 15, fontWeight: 900, color: '#1e293b', textAlign: 'right' }}>{c.score}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {(kwRows.slice(0, 2).length > 0 ? kwRows.slice(0, 2) : [{ keyword: 'Primary keyword', rank: overallScore }, { keyword: 'Secondary keyword', rank: Math.round(overallScore * 1.1) }]).map((kw, idx) => (
                <Card key={idx} style={{ background: '#f8fafc' }}>
                  ...
                  <p style={{ margin: '0 0 12px', fontSize: 10, color: '#64748b' }}>
                    Avg Rank: <strong style={{ color: '#1e293b' }}>{kw.rank}</strong>
                  </p>
                  {/* Rank dot grid */}
                  <div style={{
                    ...
                    {Array.from({ length: 15 }).map((_, j) => {
                      const drift = [-3, 0, 2, -1, 3, -2, 1, 0, -3, 2, -1, 3, 0, -2, 1][j];
                      const rank = Math.max(1, kw.rank + drift);
