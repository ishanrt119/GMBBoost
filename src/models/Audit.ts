...
export interface ICompetitor {
  name: string;
  score: number;      // rating * 10 (kept for compatibility)
  avgRank: number;    // real SERP avg rank position across keywords (lower = better)
  reviews: number;
  rating: number;
...
export interface IRealMetrics {
  ...
  overallAvgRank: number;       // average SERP rank across all keywords (e.g. 18.9)
  seoScore: number;             // profile SEO score based on keyword presence (0-100)
  missingKeywordLocations: string[]; // where top keywords are missing (Title, Additional Category, etc.)
  hasWebsite: boolean;
  hasPhone: boolean;
...
const RealMetricsSchema = new Schema<IRealMetrics>({
  ...
  overallAvgRank: { type: Number, default: 0 },
  seoScore: { type: Number, default: 0 },
  missingKeywordLocations: { type: [String], default: [] },
